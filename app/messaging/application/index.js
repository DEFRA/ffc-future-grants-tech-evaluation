...
async function getGrants(sessionId, msgQueueSuffix, userData, grantID = null) {
  ...
  const fetchGrantsMsgType = grantID ? fetchApplicationRequestMsgType : fetchGrantsRequestMsgType
  
  const {grantRequestQueueAddress, grantResponseQueueAddress }= getGrantReqResQueueAddress(msgQueueSuffix)
  await sendMessage(msgBody, fetchGrantsMsgType, grantRequestQueueAddress , { sessionId })
...
async function grantSubmitted(msgQueueSuffix, msgBody) {
  ...
  await sendMessage(msgBody, fetchGrantsRequestMsgType, grantRequestQueueAddress)
